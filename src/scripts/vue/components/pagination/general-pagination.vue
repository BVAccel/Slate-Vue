...
<template >
  <div class="pagination-wrapper">
    <!-- PAGINATION BUTTONS -->
    <!-- Next -->
    <icon-button icon="fas fa-chevron-left" @buttonClick="prevPage"></icon-button>
    <!-- Page Numbers  -->
    <button
      class="pagination-button"
      @click="pageNumberClick(page)"
      :class="{active: page == activePage }"
      v-for="(page,index) in pageNumberArray"
      :key="index"
    >{{page}}</button>
    <!-- Prev -->
    <icon-button icon="fas fa-chevron-right" @buttonClick="nextPage"></icon-button>
  </div>
</template>
